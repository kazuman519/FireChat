...
static NSString * const kMessageViewUserId = @"10";
static NSString * const kMessageViewBotId = @"11";
...
- (void)viewDidLoad {
    ...
    // firebase manager初期化
    _fbManager = [[FireBaseManager alloc] initWithId:_userID bot:_botID observer:self callback:@selector(reqMessage)];
...
- (void)didPressSendButton:(UIButton *)button
    ...
    [_fbManager setFbValue:@{@"user_id" : senderId,
                         @"message" : text,
...
-(void)reqMessage {
    ...
    // メッセージ情報をQuery
    [_fbManager reqMessageQuery:@selector(resultQuery:) observer:self];
}
